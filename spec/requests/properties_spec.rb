...
RSpec.describe 'Properties API' do
  ...
  let!(:properties) { create_list(:property, 20, user_id: user.id) }
  let(:user_id) { user.id }
  ...
  describe 'POST /users/:user_id/properties' do
    ...
    let(:valid_attributes) { { title: 'Property test.', description: 'Casa en arriendo en Santiago.', bedrooms: 2, bathrooms: 1, price: 100000, build_mtrs: 200, total_mtrs: 200, property_type: 'casa', operation: 'arriendo', state: 'usada', currency: 'clp' }.to_json }
